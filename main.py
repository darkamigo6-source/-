...
# --- ГЕНЕРАТОР PDF КАК НА ВТОРОМ СКРИНШОТЕ ---
def create_single_pdf(inventory, out_inv, period_text):
    ...
    def draw_block(title, items, x, start_y, bg_color):
        ...
        c.rect(x, curr_y-15, col_w, 15, fill=1, stroke=1)
        c.setFillColor(colors.black)
        c.setFont(FONT_NAME, 9)
        c.drawCentredString(x + col_w/2, curr_y - 11, title)
        ...
        if not items:
            ...
            c.drawString(x+5, curr_y-10, "Нет данных")
            curr_y -= 12
        else:
            for i, (name, qty) in enumerate(items):
                ...
                if i % 2 == 1:
                    ...
                    c.rect(x, curr_y-12, col_w, 12, fill=1, stroke=0)
        ...
        c.drawRightString(x+col_w-3, curr_y-12, str(int(total)) if total.is_integer() else f"{total}")
    ...
    out_list = sorted([(k, v) for k, v in out_inv.items()])

    # Рисуем Приход (Слева)
    y_l, s1 = draw_block("ПРИХОД: УЧАСТОК 1", p1, margin, y_start, "#E8F0FE")
    y_l, s2 = draw_block("ПРИХОД: УЧАСТОК 2", p2, margin, y_l, "#E8F0FE")
    
    # Рисуем Отгрузку (Справа)
    y_r, s_out = draw_block("ОТГРУЗКА (OUT)", out_list, margin + col_w + 15, y_start, "#FFF2CC")

    # Итоговая серая панель в самом низу
    footer_y = 60
    c.setStrokeColor(colors.black)
    c.setFillColor(colors.HexColor("#F3F3F3"))
    c.rect(margin, footer_y, w - margin*2, 20, fill=1, stroke=1)
    c.setFillColor(colors.black)
    c.setFont(FONT_NAME, 10)
    summary_text = f"ПРИХОД: {int(s1+s2)} | ОТГРУЗКА: {int(s_out)}"
    c.drawCentredString(w/2, footer_y + 6, summary_text)

    c.showPage(); c.save(); buffer.seek(0)
    return buffer

# --- ХЕНДЛЕРЫ И ЛОГИКА (БЕЗ ИЗМЕНЕНИЙ) ---
...
async def start(m: types.Message):
    ...
    await m.answer("📦 Отчеты приведены к нужному виду.", reply_markup=kb)
