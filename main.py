...
def aggregate_data(start_date, end_date):
    ...
    # Сбор из Заказы_Бот (добавляем то, что ты писал руками)
    try:
        bot_sheet = sh.worksheet("Заказы_Бот")
...
async def start(m: types.Message):
    ...
    await m.answer("📦 Бот готов к работе. Присылайте заказы или запрашивайте отчеты.", reply_markup=kb)

# ГЛАВНЫЙ ОБРАБОТЧИК ТЕКСТА (ЗАКАЗОВ)
@dp.message(F.text)
async def handle_text_messages(m: types.Message):
    text = m.text.strip()
    # Игнорируем нажатия кнопок отчета
    ...
    # Логика разбора заказа: "Участок 1 Полка 100х200 5"
    # Регулярка ищет слово "Участок" (или уч, уч-к) + номер, затем название, в конце число
